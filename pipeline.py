...
# =====================================================================
def resolve_refs(schema: dict, defs: Optional[dict] = None, is_properties_map: bool = False) -> dict:
    """
    Recursively resolves $ref references and merges allOf blocks in a JSON schema,
    filtering for only the fields supported by google.generativeai's protobuf Schema.
    """
    if defs is None:
    ...
    if isinstance(schema, dict):
        ...
        if "allOf" in schema:
            ...
            for k, v in schema.items():
                if k != "allOf" and k != "$ref":
                    merged[k] = resolve_refs(v, defs) if isinstance(v, (dict, list)) else v
            return resolve_refs(merged, defs)
        
        new_schema = {}
        if is_properties_map:
            for k, v in schema.items():
                new_schema[k] = resolve_refs(v, defs, is_properties_map=False)
        else:
            ALLOWED_KEYS = {"type", "format", "description", "nullable", "items", "properties", "required", "enum", "example"}
            for k, v in schema.items():
                if k in ALLOWED_KEYS:
                    new_schema[k] = resolve_refs(v, defs, is_properties_map=(k == "properties"))
        return new_schema
        
    elif isinstance(schema, list):
        return [resolve_refs(item, defs, is_properties_map=False) for item in schema]
...
def run_gemini_pipeline(audio_path: str, api_key: str, model_name: str = "gemini-3.5-flash") -> dict:
    ...
    generation_config = genai.types.GenerationConfig(
        ...
        response_schema=resolve_refs(PipelineResult.model_json_schema()),
        temperature=0.2
    )
    
    prompt = """
    You are a highly capable AI meeting intelligence agent.
    Your task is to analyze this audio file and return a structured report.
...
def answer_rag_query(query: str, transcript: list, analysis: dict, context_doc_text: Optional[str], provider: str, api_key: str, model_name: Optional[str] = None) -> str:
    ...
    if provider == "Gemini":
        genai.configure(api_key=api_key)
        model = genai.GenerativeModel(model_name if model_name else "gemini-3.5-flash")
